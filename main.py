...
@app.post("/add-review")
async def add_review(new_review: ReviewCreate, db: Session = Depends(get_async_session ) ,token: dict = Depends(verify_token)):
    review = insert(Review).values(message=new_review.message, user_id=token.get('user_id'),product_id=new_review.product_id, star=new_review.star.star, review_at=new_review.review_at)
...
@app.get('/product/reviews', response_model=List[ReviewGet])
async def get_reviews(token: dict = Depends(verify_token), db: Session = Depends(get_async_session)):
    async with db as session:
        if token is None:
            raise HTTPException(select_code=403, message="Forbidden")


        review_query = select(Review).where(Review.user_id == token('user_id'))
...
async def add_like(new_like: LikeScheme, db: Session = Depends(get_async_session), token: dict = Depends(verify_token)):
    try:
        ...
        raise HTTPException(select_code=404, message="Invalid data provided")
...
# @app.post('/add-wishlist')
# async def add_wishlist(new_wishlist: WishlistScheme, db : Session = Depends(get_async_session)):
#     query = insert(Wishlist).values(**dict(new_wishlist))
